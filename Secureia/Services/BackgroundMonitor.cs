...
// Main AI - Comandante principal que orquesta las AIs expertas (ExpertMalwareAI, ExpertNetworkAI)
...
public class BackgroundMonitor : IDisposable
{
    ...
    public void Start()
    {
        ...
        if (_expertNetwork != null)
        {
            _expertNetwork.NetworkAlert += entry =>
            {
                ...
                {
                    // NOTA: NO llamar a AssessThreat aquí porque eso puede activar el escudo
                    // en nivel 2 o 3, bloqueando TODO el tráfico de red y dejando la PC
                    // sin internet. La IP específica ya fue bloqueada por BlockIp() dentro
                    // de EmitNetworkAlert(). Eso es suficiente.
                    // 
                    // Una alerta de red NUNCA debe activar el escudo de defensa general.
                    // El escudo solo se activa por amenazas locales confirmadas (ransomware,
                    // rootkit, etc.) donde aislar el equipo tiene sentido.
                }
            };
        }
    ...
    private void AssessOverallSecurity()
    {
        ...
        var threatCount = _expertMalware != null
            ? _ransomwareFileCount.Values.Sum()
            : 0;
        ...
        try
        {
            ...
            // NOTA: En entornos corporativos es NORMAL tener cientos de conexiones
            // y decenas de puertos en escucha. NO activar escudo por esto.
            // Solo activar si hay evidencia concreta de amenaza (ransomware).
            // Los umbrales se elevan drásticamente para evitar falsos positivos
            // que dejen la PC sin conexión.
            if (listeners.Length > 250 && connections.Length > 800)
            {
                _shield.ActivateShield(2, "Número extremadamente alto de puertos/conexiones");
            }

            if (_ransomwareFileCount.Values.Any(v => v >= 10))
            {
                _shield.ActivateShield(3, "Actividad de ransomware confirmada");
            }
        }
        catch { }
    }
    ...
    {
        _shield?.AssessThreat(reason, level);
